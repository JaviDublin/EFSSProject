...
-- =============================================
CREATE PROCEDURE [dbo].[spSalesPDFUK]
		@countryId			INT			= NULL,
		@companyId			INT			= NULL,
		@buyerId			INT			= NULL,
...
BEGIN
	...
	(
		...
		VAT_PCT				VARCHAR(50)		,	
		CAPITAL_COST		VARCHAR(50)		,	BOOK_VALUE				VARCHAR(50)	,
		...
		INVOICE_DATE		VARCHAR(20)		,	COUNTRYVAT				FLOAT		,	
		SALE_TYPE			VARCHAR(5)		,	OLD_INVOICE_NUMBER		VARCHAR(20)
	...
	BEGIN
		...
		CONVERT(VARCHAR,CONVERT(MONEY,sl.CapitalCost),1), 
		CONVERT(VARCHAR,CONVERT(MONEY,sl.BookValue),1)	, 
		...
		'RV ' + CASE WHEN CONVERT(FLOAT,sl.BuyBackCap) = 0 THEN '0' ELSE
					CONVERT(VARCHAR,ROUND(
						( CONVERT(FLOAT,sl.BuyBackCap - sl.Depreciation) / 
							CONVERT(FLOAT,sl.BuyBackCap) ) *100,1)) END + ' %' ,
		CONVERT(VARCHAR(11),sl.InvoiceDate,103) ,
					cn.CountryVat			,
					sl.SaleType				,
					CONVERT(INT,sl.OldInvoiceNumber)
	...
	SELECT 
		DOCITEMID			,	DOCUMENTID			,	COMPANYID			,	DOCUMENT_NUMBER		, 
		DOCUMENT_TYPE		,	DOCUMENT_SUB_TYPE	,	COMPANY_CODE		,	COMPANY_NAME		, 
		COMPANY_FISCAL_CODE	,	COMPANY_ADDRESS_1	,	COMPANY_ADDRESS_2	,	COMPANY_ADDRESS_3	,	
		COMPANY_ADDRESS_4	,	COMPANY_ADDRESS_5	,	BUYER_ID			,	BUYER_CODE			, 
		BUYER_NAME			,	BUYER_TAX_CODE		,	BUYER_FISCAL_CODE	,	
		BUYER_ADDRESS_1		,	BUYER_ADDRESS_2		,	BUYER_ADDRESS_3		,	BUYER_ADDRESS_4		, 
		VEHICLE_SERIAL		,	VEHICLE_UNIT		,	INVOICE_NET			,	INVOICE_VAT			, 
		INVOICE_TOTAL		,	VEHICLE_PLATE		,	IN_SERVICE_DATE		,	OUT_SERVICE_DATE	, 
		MSO_DATE			,	MANUFACTURER_NAME	,	MODEL_DESCRIPTION	,	VAT_PCT				, 
		CAPITAL_COST		,	BOOK_VALUE			,	BUYBACK_CAP			,	DEPRECIATION		, 
		MILEAGE				,	DAYS_IN_SERVICE		,	DAYS_MSO			,	EXTRAS_TOTAL		, 
		TODAY				,	INVOICE_DET			,	INVOICE_DATE		,	COUNTRYVAT			,    
		SALE_TYPE			,
		@invoiceMode		AS "INVOICE_MODE",
		@freeFormatText		AS "FREE_FORMAT_TEXT"	,	OLD_INVOICE_NUMBER
	FROM 
		@TABLE_OUTPUT 
