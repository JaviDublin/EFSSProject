...
-- =============================================
CREATE PROCEDURE [dbo].[spSalesPDFFR]
		@type				INT			= NULL,
		@countryId			INT			= NULL,
		@companyId			INT			= NULL,
		@buyerId			INT			= NULL,
...
BEGIN
	...
	(
		...
		EXPORT				VARCHAR(100)	,	MSO_DATE				VARCHAR(11)	,
		HEADER_DATE			VARCHAR(255)    ,	AMOUNT7					VARCHAR(200),
		OLD_INVOICE_NUMBER	VARCHAR(255)	,
	...
	UPDATE @TABLE_OUTPUT SET EXPORT		= ''  WHERE EXPORT IS NULL
	...
	SET @header  = 'Date: ' + @day + ' ' +  @month + ' ' + @year 
	
	--UPDATE @TABLE_OUTPUT SET HEADER_DATE = 'FACTURE N° ' + DOCUMENT_NUMBER + '        ' +  @header
	--WHERE DOCUMENT_TYPE = 'INVOICE'
	
	--UPDATE @TABLE_OUTPUT SET HEADER_DATE = 'AVOIR ' + DOCUMENT_NUMBER + '        ' +  @header +
	--'     Annule facture ' + OLD_INVOICE_NUMBER
	--WHERE DOCUMENT_TYPE = 'CREDIT NOTE'
	
	UPDATE @TABLE_OUTPUT SET HEADER_DATE = 'FACTURE N° ' + DOCUMENT_NUMBER + '        ' +  'Date: ' +
	CONVERT(VARCHAR,DAY(CONVERT(DATETIME,INVOICE_DATE,103))) + ' ' +
	DATENAME(MONTH,CONVERT(DATETIME,INVOICE_DATE,103))       + ' ' +
	CONVERT(VARCHAR,YEAR(CONVERT(DATETIME,INVOICE_DATE,103)))
	
	WHERE DOCUMENT_TYPE = 'INVOICE'
	
	UPDATE @TABLE_OUTPUT SET HEADER_DATE = 'AVOIR ' + DOCUMENT_NUMBER + '        ' +  'Date: ' +
	CONVERT(VARCHAR,DAY(CONVERT(DATETIME,INVOICE_DATE,103))) + ' ' +
	DATENAME(MONTH,CONVERT(DATETIME,INVOICE_DATE,103))       + ' ' +
	CONVERT(VARCHAR,YEAR(CONVERT(DATETIME,INVOICE_DATE,103))) +
	'     Annule facture ' + OLD_INVOICE_NUMBER
	...
	SELECT
		...
		COMPANY_ADDRESS_9	,	COMPANY_ADDRESS_10		,
		BUYER_ID			,	BUYER_CODE				,
