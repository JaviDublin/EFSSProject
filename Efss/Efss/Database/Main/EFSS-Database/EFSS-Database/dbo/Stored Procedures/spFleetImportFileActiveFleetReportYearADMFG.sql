...
-- =============================================
CREATE PROCEDURE [dbo].[spFleetImportFileActiveFleetReportYearADMFG]
...
BEGIN
	...
	SELECT t.[count] as 'Count' , t.ManufacturerId , 
	t.CountryCode ,
	t.ManufacturerName , t.Total ,
	...
	(
    SELECT 
    COUNT(mf.ManufacturerId) OVER() AS 'Count',
    ROW_NUMBER() OVER
			(
				ORDER BY 
				CASE WHEN @sortExpression = 'CountryCode' THEN cn.CountryCode END ASC,
				CASE WHEN @sortExpression = 'CountryCode DESC' THEN cn.CountryCode END DESC,
				CASE WHEN @sortExpression = 'ManufacturerName' THEN mf.ManufacturerName END ASC,
				CASE WHEN @sortExpression = 'ManufacturerName DESC' THEN mf.ManufacturerName END DESC,
    ...
    mf.ManufacturerId  ,
    cn.CountryCode ,
    mf.ManufacturerName , SUM(fy.Total) as "Total" ,
	...
	INNER JOIN [Dimension.Manufacturer] as mf ON
			fy.ManufacturerId = mf.ManufacturerId
	where fy.FileId = @fileId and fy.DateYear = @year and fy.CountryId = @countryId
	group by  mf.ManufacturerId  ,mf.ManufacturerName , cn.CountryCode
