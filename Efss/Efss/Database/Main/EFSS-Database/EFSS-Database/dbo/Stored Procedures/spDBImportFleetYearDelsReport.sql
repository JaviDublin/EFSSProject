...
-- =============================================
CREATE PROCEDURE [dbo].[spDBImportFleetYearDelsReport]
...
BEGIN
	...
	BEGIN
			...
			USING										
				(
					SELECT
							afyd.CountryId , afyd.ManufacturerId , afyd.LogId , afyd.FileId , 
							MONTH(GETDATE())			AS "MonthNumber", 
							YEAR(GETDATE())				AS "DateYear", 
							GETDATE()					AS "DateCreated",
							COUNT(*)					AS "Total",
							COALESCE(afydrp.Total,0)	AS "ReportTotal",
							CASE WHEN afyd.ManufacturerId = 45	THEN 
								COUNT(*)						ELSE
							COUNT(*) - COALESCE(afydrp.Total,0) END AS "NewTotal"
						FROM  [Staging.ActiveFleetYearDeletes] afyd
						LEFT JOIN [Import.ActiveFleetYearADReport] AS afydrp ON
								afydrp.DateYear			= YEAR(GETDATE()) 
							AND afydrp.FileId			= afyd.FileId
							AND afydrp.CountryId		= afyd.CountryId 
							AND afydrp.ManufacturerId	= afyd.ManufacturerId
						GROUP BY 
							afyd.CountryId , afyd.ManufacturerId , afyd.LogId , 
							afyd.FileId , afydrp.Total
			) AS SOURCE
			ON 
			(
